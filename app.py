...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('milkbasket_scheduler.log'),
...
class MilkbasketAutomation:
            ...
            for i, email in enumerate(emails, 1):
                try:
                    ...
                    if inv_match:
                        ...
                        if '/' in inv_number:
                            self.log(f"[SKIPPED] Health Factory email (contains '/' in invoice): {subject[:50]}")
                            continue
    ...
    def process_extracted_data(self, extracted_data: Dict, file_info: Dict) -> List[Dict]:
        ...
        field_mappings = {
            ...
            "grn_number": ["grn_no", "grn_number", "GRN No", "GRN Number"],
            "grn_date": ["grn_date", "delivered_on", "GRN Date", "delivery_date"],
            "vendor_invoice_number": ["invoice_no", "vendor_invoice_number", "invoice_number", "inv_no", "Invoice No"],
